...
#### html.ts (типы для работы с html)
- SelectorElement<T> – тип, который представляет собой либо значение типа T, либо строку. Используется для задания селектора элемента в DOM, который может быть как строкой (например, селектор CSS), так и объектом типа T.

- SelectorCollection<T> – тип для представления коллекции элементов. Может быть строкой (CSS-селектор), коллекцией элементов (NodeListOf<Element>) или массивом элементов типа T.

- ElementChild – тип для описания дочерних элементов элемента. Может быть либо одним элементом HTMLElement, либо массивом таких элементов.

- ElementAttrs – тип для перечисления атрибутов, которые могут быть настроены для элементов. Включает атрибуты как textContent, className, href, src, alt и dataset. Эти атрибуты могут быть изменены для настройки элемента.

- ElementProps<T extends HTMLElement> – тип для представления свойств элемента HTMLElement. Используется для задания свойств для любого элемента HTML, с возможностью опциональной настройки всех полей через Partial. Тип поддерживает строковые, логические и объектные значения для каждого свойства.

- ElementValue<T extends HTMLElement> – тип для значений, которые могут быть установлены для элемента. Может быть строкой, дочерними элементами (ElementChild), или объектом свойств элемента (ElementProps<T>).

- ElementCreator<T extends HTMLElement = HTMLElement> – тип для описания конструктора элемента. Представляет собой кортеж из имени тега HTML (например, 'div', 'a' и т.д.) и объекта свойств для элемента этого типа.
